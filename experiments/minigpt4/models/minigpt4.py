...
class MiniGPT4(MiniGPTBase):
    ...
    # prompt is used with Q-Former instruction
    def encode_img_prompt_instruction(self, image, prompt, preprompt):
        device = image.device
        ...
        with self.maybe_autocast():
            image_embeds = self.ln_vision(self.visual_encoder(image)).to(device)
            assert self.has_qformer == True; "Prompt should be used with Q-Former"
            image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(device)
            ...
            if preprompt is not None:
                ...
                inputs_llm_cd = self.llama_proj(query_output_cd.last_hidden_state[:,:query_tokens.size(1),:])
                atts_llm_cd = torch.ones(inputs_llm_cd.size()[:-1], dtype=torch.long).to(image.device)
            return inputs_llm, atts_llm, inputs_llm_cd, atts_llm_cd

    # default: prompt is no Q-Former instruction
    def encode_img_prompt_default(self, image,preprompt):
        device = image.device
        ...
        with self.maybe_autocast():
            ...
            inputs_llm = self.llama_proj(query_output.last_hidden_state)
            atts_llm = torch.ones(inputs_llm.size()[:-1], dtype=torch.long).to(image.device)
