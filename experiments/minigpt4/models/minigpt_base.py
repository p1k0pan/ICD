...
class MiniGPTBase(BaseModel):
    ...
    def generate(
        ...
        if prompt is not None:
            img_embeds, atts_img, img_embeds_cd, atts_img_cd = self.encode_img_prompt_instruction(images.to(self.device), prompt, preprompt)
        else:
            img_embeds, atts_img, img_embeds_cd, atts_img_cd = self.encode_img_prompt_default(images.to(self.device), preprompt)
        image_lists = [[image_emb[None]] for image_emb in img_embeds]

        batch_embs = [self.get_context_emb(text, img_list) for text, img_list in zip(texts, image_lists)]

        batch_size = len(batch_embs) #1
        max_len = max([emb.shape[1] for emb in batch_embs]) #64
        emb_dim = batch_embs[0].shape[2] # 4096
        dtype = batch_embs[0].dtype
        device = batch_embs[0].device
        # print(batch_size, max_len, emb_dim, dtype, device)
        ...
        if img_embeds_cd is not None and atts_img_cd is not None:
            ...
            batch_size = len(batch_embs_cd) #1
            max_len = max([emb.shape[1] for emb in batch_embs_cd]) #64
            emb_dim = batch_embs_cd[0].shape[2] # 4096
        ...
        with self.maybe_autocast():
            outputs = self.llama_model.generate(
                ...
                inputs_embeds_cd=embs_cd if preprompt is not None else None,
                attention_mask_cd=attn_mask_cd if preprompt is not None else None,
