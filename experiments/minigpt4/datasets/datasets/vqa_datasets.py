...
class POPEGQAData(torch.utils.data.Dataset):
    def __init__(self, loaded_data, vis_processor, root_path, format, is_default=False):
        self.loaded_data = loaded_data
        self.root_path = root_path
        self.vis_processor = vis_processor
        self.format = format
    ...
    def __getitem__(self, idx):
        ...
        if self.is_default:
            ...
            question = question + self.format

        return image, question, ann["question_id"], image_path

class MMEData(torch.utils.data.Dataset):
    def __init__(self, loaded_data, vis_processor, root_path, format):
        self.loaded_data = loaded_data
        self.root_path = root_path
        self.vis_processor = vis_processor
        self.format = format
    ...
    def __getitem__(self, idx):
        key = list(self.loaded_data.keys())[idx]
        data = self.loaded_data[key]
        try:
            image_path = os.path.join(self.root_path, key+".jpg")
            raw_image = Image.open(image_path).convert("RGB")
        except FileNotFoundError as e:
            image_path = os.path.join(self.root_path, key+".png")
            raw_image = Image.open(image_path).convert("RGB")
        image = self.vis_processor(raw_image)

        return image, data, key, image_path
