...
class Blip2VicunaInstruct(Blip2Base):
    ...
    def generate(
        ...
        if isinstance(preprompt_cd, str):
            preprompt_cd = [preprompt_cd] * bs
        ...
        if self.qformer_text_input:
            ...
            if use_cd:
                if preprompt_cd is not None:
                    ...
                    print("use cd but preprompt is None")
                    sys.exit(1)
        ...
        if image.dim() == 5:
            ...
            if self.qformer_text_input:
                ...
                if images_cd is not None:
                    query_output_image_cd = self.Qformer.bert(
                        text_Qformer.input_ids,
                        attention_mask=Qformer_atts,
                        query_embeds=query_tokens,
            ...
            if images_cd is not None:
                inputs_llm_image_cd = self.llm_proj(query_output_image_cd.last_hidden_state[:,:query_tokens.size(1),:])
                atts_llm_image_cd = torch.ones(inputs_llm_image_cd.size()[:-1], dtype=torch.long).to(image.device)
        ...
        if sub_objects is not None:
            ...
            for obj in sub_objects:
                sub_tokens.append(self.llm_tokenizer(obj, add_special_tokens=False)["input_ids"])
            sub_tokens = [item for sublist in sub_tokens for item in sublist]
        ...
        with self.maybe_autocast():
            ...
            ## cd_comments: format
            if images_cd is not None:
                inputs_embeds_image_cd = self.llm_model.get_input_embeddings()(llm_tokens.input_ids)
                inputs_embeds_image_cd = torch.cat([inputs_llm_image_cd, inputs_embeds_image_cd], dim=1)
                attention_mask_image_cd = torch.cat([atts_llm_image_cd, llm_tokens.attention_mask], dim=1)
            ...
            outputs = self.llm_model.generate(
                ...
                inputs_embeds_cd=inputs_embeds_cd if use_cd else None,
                attention_mask_cd=attention_mask_cd if use_cd else None,
                inputs_embeds_image_cd=inputs_embeds_image_cd if images_cd is not None else None,
                attention_mask_image_cd=attention_mask_image_cd if images_cd is not None else None,
                cd_beta=cd_beta if cd_beta is not None else None,
                cd_alpha=cd_alpha if cd_alpha is not None else None,
                sub_tokens=sub_tokens if sub_tokens is not None else None
            )
    ...
    def predict_class(
        ...
        return self._predict_class(samples, candidates, n_segments, preprompt_cd, cd_alpha, cd_beta)
    ...
    def _predict_class(
        ...
        if preprompt_cd is not None:
            empty_targets_cd = torch.ones(atts_llm_cd.size(), dtype=torch.long).to(image.device).fill_(-100)
        ...
        with self.maybe_autocast(dtype=torch.bfloat16):
            ...
            for n in range(n_segments):
                ...
                if preprompt_cd is not None:
                    inputs_embeds_cd = torch.cat([inputs_llm_cd.repeat_interleave(seg_len, dim=0), inputs_embeds], dim=1)
                    attention_mask_cd = torch.cat([atts_llm_cd.repeat_interleave(seg_len, dim=0), this_llm_atts], dim=1)
            ...
            # output_class_ranks = torch.argsort(all_logits, dim=-1)
